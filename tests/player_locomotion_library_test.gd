...
func _run() -> void:
	...
	animator._on_locomotion_speed_changed(0.0, "NONE", 5.0, 8.0)
	if animator.current_state != &"IDLE": return _fail("RUNTIME_IDLE")
	animator._on_locomotion_speed_changed(5.0, "AUTO", 5.0, 8.0)
	if animator.current_state != &"WALK": return _fail("RUNTIME_WALK")
	animator._on_locomotion_speed_changed(8.0, "KEYBOARD", 5.0, 8.0)
