...
func _run() -> void:
	...
	var config: Dictionary = component.call("lesson_config_snapshot")

	_expect(initial.get("lesson") == "PHONICS_A_LONG_SHORT", "A compatibility lesson id is preserved")
	_expect(initial.get("lessonMode") == "PHONICS_A_SORT", "A compatibility lesson mode is preserved")
	_expect(initial.get("vowel") == "A", "active vowel comes from lesson config")
	_expect(initial.get("lessonConfigVersion") == 1, "lesson config version is published")
	_expect(initial.get("wallThemeId") == "underwater_vowel_a_pending_art", "wall theme hook is published")
	_expect(initial.get("rainbowThemeId") == "shared_rainbow_v1", "shared rainbow theme hook is published")
	...
	_expect(config.get("shortLabel") == "SHORT A", "short label comes from config")
	_expect(config.get("longLabel") == "LONG A", "long label comes from config")
	_expect(component.get_node_or_null("ShortASortBox") != null, "short-A sorting box remains compatible")
	_expect(component.get_node_or_null("LongASortBox") != null, "long-A sorting box remains compatible")
